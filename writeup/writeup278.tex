...
As stated above the log records are created and written to a log file within each database function. The data field within the log record is produced by a bitwise XOR between the new state of the record and the old. 

\subsubsection{Log Roll Back}

Log roll back is implemented in two different functions, linear and parallel. In the case of the linear method it simply goes through the list of modified record summaries and applies each to the corresponding location in either the original database or a clone using the standard BerkeleyDB get and put commands.
